...
import { mockResponseData, emergencyResponseData } from '../test-data';
...
export default function ResultsDemo() {
  ...
  const data = showEmergency ? emergencyResponseData : mockResponseData;
  ...
  if (data.emergency_detected) {
    return (
      <Layout>
        ...
        <div className="fixed top-20 right-4 bg-white rounded-lg shadow-lg p-4 border border-gray-200">
          <button
            onClick={() => setShowEmergency(!showEmergency)}
            className="text-sm font-medium text-blue-600 hover:text-blue-800"
          >
            {showEmergency ? 'Show Normal Response' : 'Show Emergency Response'}
          </button>
        </div>
      </Layout>
    );
